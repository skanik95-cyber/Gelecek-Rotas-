export const careerData = [
    {
        name: "Doktor",
        description: "İnsan sağlığını koruma, hastalıkları teşhis ve tedavi etme görevi olan, tıp eğitimi almış profesyonel.",
        requirements: {
            "Sayısal Yetenek": 2.5,
            "Göz-El Koordinasyonu": 2.5,
            "Fen Bilimleri İlgisi": 3.5,
            "Sosyal Yardım İlgisi": 3.5
        },
        vr_image: ""
    },

    {
        name: "Öğretmen",
        description: "Öğrencilere bilgi ve beceri kazandıran, eğitim süreçlerini yöneten, pedagojik formasyona sahip kişi.",
        requirements: {
            "Sözel Yetenek": 3.5,
            "Sosyal Bilimler İlgisi": 3.5,
            "Sosyal Yardım İlgisi": 2.5,
            "İkna İlgisi": 2.5
        },
        vr_image: "images/teacher_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=_y-lSw1SNaU",
        vrDescription: "Öğretmenlik mesleğini 360° VR video ile deneyimleyin."
    },

    {
        name: "Veteriner",
        description: "Hayvanların sağlık sorunlarını inceleyen, tedavi eden ve koruyucu sağlık hizmeti sunan uzman.",
        requirements: {
            "Fen Bilimleri İlgisi": 4.0,
            "Sayısal Yetenek": 3.0,
            "Sosyal Yardım İlgisi": 3.5,
            "Göz-El Koordinasyonu": 3.0,
            "Ziraat İlgisi": 3.0,
            "Mekanik İlgi": 2.0
        },
        vr_image: "images/veterinary_clinic_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=EGDXVQwOGf0"
    },

    {
        name: "Astronot",
        description: "Uzay görevlerine katılan, deneyler yapan ve uzay araçlarını kullanan uzman.",
        requirements: {
            "Fen Bilimleri İlgisi": 4.0,
            "Sayısal Yetenek": 3.5,
            "Şekil-Uzay Yeteneği": 4.0,
            "Mekanik İlgi": 3.5,
            "Göz-El Koordinasyonu": 3.5,
            "Yabancı Dil İlgisi": 3.0
        },
        vr_image: "images/astronaut_spacewalk_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=hEdzv7D4CbQ&t=5s"
    },

    {
        name: "Çocuk Gelişim Uzmanı",
        description: "Çocukların gelişimini değerlendiren ve destek programları oluşturan uzman.",
        requirements: {
            "Sosyal Yardım İlgisi": 4.0,
            "Sosyal Bilimler İlgisi": 3.5,
            "Sözel Yetenek": 3.5,
            "İkna İlgisi": 3.0,
            "Göz-El Koordinasyonu": 2.0,
            "Fen Bilimleri İlgisi": 2.0
        },
        vr_image: "images/child_development_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=Fdx3JdWlOls"
    },

    {
        name: "Ressam",
        description: "Sanat eserleri üreten, duygularını tuvalle ifade eden sanatçı.",
        requirements: {
            "Güzel Sanatlar İlgisi": 4.0,
            "Sözel Yetenek": 2.5,
            "Şekil-Uzay Yeteneği": 3.5,
            "İş Ayrıntıları İlgisi": 3.0
        },
        vr_image: "images/painter_studio_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=pq2TwRNy_FU"
    },

    {
        name: "İtfaiyeci",
        description: "Yangınlara müdahale eden, kurtarma yapan kamu güvenliği çalışanı.",
        requirements: {
            "Göz-El Koordinasyonu": 4.0,
            "Mekanik İlgi": 3.0,
            "Şekil-Uzay Yeteneği": 3.0,
            "Sosyal Yardım İlgisi": 4.0,
            "Fen Bilimleri İlgisi": 2.5,
            "Sayısal Yetenek": 2.0
        },
        vr_image: "images/firefighter_rescue_simulation.png",
        hasVRVideo: true,
        vrVideoFile: "https://www.youtube.com/watch?v=W5njz511s1k"
    }
];
